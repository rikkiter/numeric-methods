...
def str_to_decimal_and_round(*args: str, round_pattern: Decimal):
    ...
    for i in range(len(args)):
        res.append(Decimal(args[i]).quantize(round_pattern))
    return res


def compute_expression(a: str, b: str, c: str):
    decimal_place = accuracy(a, b, c)
    round_pattern = Decimal("1." + "0" * (decimal_place + 1))
    answer_round_pattern = Decimal("1." + "0" * decimal_place)
    a, b, c = str_to_decimal_and_round(a, b, c, round_pattern=round_pattern)
